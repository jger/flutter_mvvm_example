...
- **Coverage badge** (CI), first time:
  ...
  6. Push to **main**/**master** (or open an internal PR) so **Flutter CI** runs; then **Actions → Flutter CI → latest run → Summary**: copy the **README** line, or replace **`GIST_ID_PLACEHOLDER`** in this file with your gist ID.
  If Shields shows **resource not found**, the placeholder is still in the README or CI hasn’t written **`coverage.json`** to the gist yet. Fork PRs: no secrets → badge step skipped.
